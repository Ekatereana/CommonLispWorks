...
(defun make_query_table (basic columns distinct condition)
  ...
  (let ((col_ids);; get numbers of columns that should be displayed in new table
    ...
    ;; where
    (if condition
        (setq b_rows (where b_rows (table-rows_names basic) condition) ))
  ...
  (defun select (statement)
    (let (
      ...
      (cond
        ...
        ((string= (car (select-statement-columns query)) "*")
         (pretty_view table))
        (t (pretty_view (make_query_table table (select-statement-columns query) (select-statement-is_distinct query) (select-statement-condition query))))
        )
      ))
